...
| `/admin/login`, `/admin/*` | Same .NET host, React SPA; protected server route and APIs | Private, no-store, noindex |
| `/api/*`, `/media/{id}/{variant}` | .NET | API responses or immutable allowlisted image variants |
...
Admin login uses a 12-hour HttpOnly SameSite cookie backed by persistent .NET Data Protection keys and a double-submit CSRF token. Changes to destinations/hotels/rooms/offers/posts, publication state, inbox statuses, rich documents and uploaded image variants use the same-origin API. The initial admin credentials seed only when no admin exists; reseeding does not rotate them or overwrite edited catalog rows. Uploaded media and keys must live on persistent storage.

The `/admin` login and protected subpages use the shared Razor site header and footer around the admin application. They remain private/no-store and noindex, without canonical URLs; the login page shows a JavaScript-required notice if scripting is disabled.
